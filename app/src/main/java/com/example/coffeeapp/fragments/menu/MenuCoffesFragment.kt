package com.example.coffeeapp.fragments.menu

import android.os.Bundle
import androidx.fragment.app.Fragment
import android.view.LayoutInflater
import android.view.View
import android.view.ViewGroup
import com.example.coffeeapp.R
import com.example.coffeeapp.databinding.FragmentEnterBinding
import com.example.coffeeapp.databinding.FragmentMenuCoffesBinding
import com.example.coffeeapp.viewmodel.CoffeeViewModel

class MenuCoffesFragment : Fragment() {

    private lateinit var binding: FragmentMenuCoffesBinding
    private lateinit var viewModel: CoffeeViewModel

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        binding = FragmentMenuCoffesBinding.inflate(inflater)

        return binding.root
    }

}